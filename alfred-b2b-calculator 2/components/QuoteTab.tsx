...
export const QuoteTab: React.FC<QuoteTabProps> = ({ state, onChange }) => {
  ...
  const calculateCosts = () => {
    ...
    // Plan = "Basico" → Base = 0.26
    // Plan = "Pro" → Base = 0.20
    // Plan = "Enterprise" → Base = 0.15
    let Base = 0.26;
    ...
    // Escala por flota (a mayor N, menor ratio, con rendimientos decrecientes)
    // Adj_N = 0.03 * LOG10( MAX(1, N) ) - 0.03 * LOG10(20)
    const valN = Math.max(1, N);
    const Adj_N = (0.03 * Math.log10(valN)) - (0.03 * Math.log10(20));

    // Talleres propios (más capacidad de ejecución interna = menor ratio)
    // rT = MIN(1, T / 3) -> Adj_T = 0.02 * rT
    const rT = Math.min(1, T / 3);
    const Adj_T = 0.02 * rT;

    // In-Plant (más adopción y disciplina = menor ratio)
    // Adj_I = 0.02 * I
    const Adj_I = 0.02 * I;

    // 3. Calculate Target Ratio
    // Ratio_Objetivo = Base - Adj_N - Adj_T - Adj_I
    let Ratio_Objetivo = Base - Adj_N - Adj_T - Adj_I;

    // 4. Clamp Ratio (10% - 30%)
    // Ratio_Objetivo = MIN(0.30, MAX(0.10, Ratio_Objetivo))
    Ratio_Objetivo = Math.min(0.30, Math.max(0.10, Ratio_Objetivo));

    // 5. Calculate Final Values
    // Ahorro_Tecnico_Estimado = Inversion_Anual_Total / Ratio_Objetivo
    ...
    if (Inversion_Anual_Total > 0) {
      ...
      // Porcentaje_Inversion = Inversion_Anual_Total / Impacto_Financiero_Proyectado
      // Mathematically, this should return Ratio_Objetivo * 100, assuming strict math, 
      // but we calculate it explicitly to handle rounding.
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="p-4 flex flex-col h-full">
                        ...
                        {/* Button Visual */}
                        <div className={`mt-4 text-center py-1.5 rounded text-xs font-semibold transition-all ${
                          isSelected 
                            ? plan.buttonBg 
            ...
            <div className="bg-alfred-dark/50 p-4 rounded-xl border border-white/5 flex items-center justify-between group hover:border-white/10 transition-colors">
              ...
              <button
                ...
                className={`relative inline-flex h-7 w-12 items-center rounded-full transition-colors ${
                  state.includeInPlant ? 'bg-alfred-lime' : 'bg-gray-700'
                }`}
              >
                <span
                  className={`inline-block h-5 w-5 transform rounded-full bg-white transition-transform ${
                    state.includeInPlant ? 'translate-x-6' : 'translate-x-1'
                  }`}
                />
              </button>
            </div>
        ...
        <div className="bg-alfred-surface/50 p-6 rounded-2xl border border-white/5 shadow-xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="flex flex-col gap-2">
              ...
              <select
                className="w-full bg-alfred-dark border border-alfred-blue/30 rounded-lg px-4 py-3 text-white focus:border-alfred-lime focus:outline-none focus:ring-1 focus:ring-alfred-lime transition-all cursor-pointer"
                value={state.paymentTerm}
                onChange={(e) => onChange({ paymentTerm: e.target.value as PaymentTerm })}
              >
                <option value={PaymentTerm.NET0_30}>0 - 30 días</option>
                <option value={PaymentTerm.NET31_60}>31 - 60 días</option>
              </select>
            </div>
             ...
             <div className="flex flex-col gap-2">
              ...
              <select
                ...
                onChange={(e) => onChange({ contractDuration: parseInt(e.target.value) as ContractDuration })}
              >
                <option value={ContractDuration.NONE}>Seleccionar...</option>
                <option value={ContractDuration.ONE_YEAR}>1 Año (Ahorras 5%)</option>
                <option value={ContractDuration.TWO_YEARS}>2 Años (Ahorras 15%)</option>
                <option value={ContractDuration.THREE_YEARS}>3 Años (Ahorras 20%)</option>
              </select>
            </div>
          </div>
        </div>
      ...
      <div className="lg:col-span-5">
        <div className="sticky top-8 bg-alfred-surface border-t-4 border-alfred-lime rounded-2xl shadow-2xl p-6 lg:p-8 space-y-6">
          ...
          <div className="space-y-4">
            ...
            {/* Variable Tech - Shows DYNAMIC Unit Price based on contract duration */}
            <div className="flex justify-between items-center py-3 border-b border-white/10">
              <div className="flex items-center gap-2">
                <div className="p-1.5 bg-blue-500/10 rounded-lg text-alfred-blue">
                  <Truck size={16} />
            ...
            {state.paymentTerm === PaymentTerm.NET31_60 && (
              <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-3 mt-2">
                 <p className="text-xs text-red-300 leading-relaxed">
                   <span className="font-bold">Nota:</span> Recargo financiero (+2.5%) aplica únicamente en facturas de servicios realizados por terceros.
                 </p>
              </div>
            )}

            {/* --- NEW HIGH IMPACT SAVINGS SECTION --- */}
